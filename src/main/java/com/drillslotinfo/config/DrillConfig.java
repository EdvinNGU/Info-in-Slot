...
public class DrillConfig {
    ...
    public static int           chanceColor             = 0xFFFF55; // yellow default
    public static boolean       chanceBold              = false;
    public static boolean       showChanceBackground    = false;    // off by default
    public static int           chanceBgColor           = 0x000000;
    public static PriceSize     chanceSize              = PriceSize.MEDIUM; // medium default
    ...
    public static int           chanceSymbolColor       = 0;   // 0 = auto from tooltip
    public static boolean       chanceFractionMode      = false;
    public static int           chanceNumeratorColor    = 0;   // 0 = auto (match digit)
    public static int           chanceDenominatorColor  = 0;   // 0 = auto
    public static int           chanceSlashColor        = 0;   // 0 = auto
    ...
    public static ConfigBuilder getConfigBuilder() {
        ...
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("Enable Mod"), enabled)
                .setDefaultValue(true).setSaveConsumer(v -> enabled = v).build());
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("Show Timer"), showTimer)
                .setDefaultValue(true).setSaveConsumer(v -> { if (v != initShowTimer) showTimer = v; }).build());
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("Show Order/Auction Price"), showOrderPrice)
                .setDefaultValue(true).setSaveConsumer(v -> { if (v != initShowOrderPrice) showOrderPrice = v; }).build());
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("Show Order Count"), showDelivered)
                .setDefaultValue(true).setSaveConsumer(v -> { if (v != initShowDelivered) showDelivered = v; }).build());
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("Toggle Coins"), showCoins)
                .setDefaultValue(true).setSaveConsumer(v -> { if (v != initShowCoins) showCoins = v; }).build());
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("Toggle Shards"), showShards)
                .setDefaultValue(true).setSaveConsumer(v -> { if (v != initShowShards) showShards = v; }).build());
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("Toggle Keys"), showKeyCount)
                .setDefaultValue(true).setSaveConsumer(v -> { if (v != initShowKeyCount) showKeyCount = v; }).build());
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("Show Currency Symbol"), showCurrencySymbol)
                .setDefaultValue(true)
                .setTooltip(Text.literal("Show/hide the $ prefix on Auction Price and Order Price"))
                .setSaveConsumer(v -> showCurrencySymbol = v).build());
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("Display Stats in Container"), showInContainer)
                .setDefaultValue(true)
                .setTooltip(Text.literal("Show slot stats in chest/shop/container screens"))
                .setSaveConsumer(v -> showInContainer = v).build());
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("Display Stats in Inventory"), showInInventory)
                .setDefaultValue(true)
                .setTooltip(Text.literal("Show slot stats in the player inventory screen (36 slots)"))
                .setSaveConsumer(v -> showInInventory = v).build());
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("Display Stats in Hotbar"), showInHotbar)
                .setDefaultValue(true)
                .setTooltip(Text.literal("Show slot stats on the hotbar when no screen is open"))
                .setSaveConsumer(v -> showInHotbar = v).build());
        mainCat.addEntry(eb.startBooleanToggle(Text.literal("All Backgrounds"), allBackgroundsOn())
                .setDefaultValue(true)
                .setTooltip(Text.literal("Toggle backgrounds for ALL displays at once"))
                .setSaveConsumer(v -> { if (v != initAllBg) allBgPending = v; }).build());
        mainCat.addEntry(new ColorSwatchEntry(Text.literal("Universal Background Color"),
                timerBgColor, 0x000000, v -> { if (!v.equals(initUniversalBgColor)) allBgColorPending = v; }));
        mainCat.addEntry(eb.startEnumSelector(Text.literal("Universal Size"), PriceSize.class, priceSize)
                .setDefaultValue(PriceSize.SMALL)
                .setTooltip(Text.literal("Set the same size for ALL displays at once"))
                .setEnumNameProvider(v -> sizeLabel((PriceSize) v))
                .setSaveConsumer(v -> { if (!v.equals(initUniversalSize)) allSizePending = v; }).build());

        // ── Order/Auction Price ───────────────────────────────────────────────
        ConfigCategory orderPriceCat = builder.getOrCreateCategory(Text.literal("Order/Auction Price"));
        orderPriceCat.addEntry(eb.startBooleanToggle(Text.literal("Show Order/Auction Price"), showOrderPrice)
                .setDefaultValue(true)
                .setTooltip(Text.literal("Show /order price ($ X) and /ah price ($X) on item slots"))
                .setSaveConsumer(v -> { if (v != initShowOrderPrice) showOrderPrice = v; }).build());
        orderPriceCat.addEntry(eb.startEnumSelector(Text.literal("Position Height"), TimerPosition.class, orderPricePosition)
        ...
        orderPriceCat.addEntry(eb.startEnumSelector(Text.literal("Position Width"), HorizPosition.class, orderPriceHorizPosition)
        ...
        orderPriceCat.addEntry(new ColorSwatchEntry(Text.literal("Background Color"),
        ...
        // ── Order Count ───────────────────────────────────────────────────────
        ConfigCategory orderCountCat = builder.getOrCreateCategory(Text.literal("Order Count"));
        orderCountCat.addEntry(eb.startBooleanToggle(Text.literal("Show Order Count"), showDelivered)
                .setDefaultValue(true)
        ...
        orderCountCat.addEntry(eb.startEnumSelector(Text.literal("Position Height"), TimerPosition.class, deliveredPosition)
        ...
        orderCountCat.addEntry(eb.startEnumSelector(Text.literal("Position Width"), HorizPosition.class, deliveredHorizPosition)
                .setDefaultValue(HorizPosition.LEFT).setEnumNameProvider(v -> horizLabel((HorizPosition) v))
                .setSaveConsumer(v -> deliveredHorizPosition = v).build());
        orderCountCat.addEntry(eb.startBooleanToggle(Text.literal("Background"), showDeliveredBackground)
                .setDefaultValue(true).setSaveConsumer(v -> showDeliveredBackground = v).build());
        orderCountCat.addEntry(new ColorSwatchEntry(Text.literal("Background Color"),
                deliveredBgColor, 0x000000, v -> deliveredBgColor = v));
        orderCountCat.addEntry(eb.startEnumSelector(Text.literal("Size"), PriceSize.class, deliveredSize)
                .setDefaultValue(PriceSize.SMALL).setEnumNameProvider(v -> sizeLabel((PriceSize) v))
                .setSaveConsumer(v -> deliveredSize = v).build());
        orderCountCat.addEntry(new ColorSwatchEntry(Text.literal("Color"), deliveredColor, 0xFFFFFF,
                deliveredBold, false, v -> deliveredColor = v, v -> deliveredBold = v));

        // ── Timer ─────────────────────────────────────────────────────────────
        ConfigCategory timerCat = builder.getOrCreateCategory(Text.literal("Timer"));
        ...
        timerCat.addEntry(new ColorSwatchEntry(Text.literal("Background Color"),
        ...
        timerCat.addEntry(eb.startEnumSelector(Text.literal("Timer Size"), TimerSize.class, timerSize)
                ...
                .setEnumNameProvider(v -> switch ((TimerSize) v) {
                    case XSMALL -> Text.literal("XSmall");
                    case SMALL  -> Text.literal("Small");
                    case MEDIUM -> Text.literal("Medium");
                    case LARGE  -> Text.literal("Large");
                })
                .setSaveConsumer(v -> timerSize = v).build());
        timerCat.addEntry(eb.startEnumSelector(Text.literal("Timer Position Height"), TimerPosition.class, timerPosition)
        ...
        timerCat.addEntry(eb.startEnumSelector(Text.literal("Timer Position Width"), HorizPosition.class, timerHorizPosition)
        ...
        // ── Buy/Sell ──────────────────────────────────────────────────────────
        ConfigCategory buySellCat = builder.getOrCreateCategory(Text.literal("Buy/Sell"));
        buySellCat.addEntry(eb.startBooleanToggle(Text.literal("Show Buy Price"), showBuyPrice)
                .setDefaultValue(true).setSaveConsumer(v -> showBuyPrice = v).build());
        buySellCat.addEntry(eb.startEnumSelector(Text.literal("Buy Position Height"), TimerPosition.class, buyPosition)
                .setDefaultValue(TimerPosition.TOP).setEnumNameProvider(v -> posLabel((TimerPosition) v))
                .setSaveConsumer(v -> buyPosition = v).build());
        buySellCat.addEntry(eb.startEnumSelector(Text.literal("Buy Position Width"), HorizPosition.class, buyHorizPosition)
                .setDefaultValue(HorizPosition.LEFT).setEnumNameProvider(v -> horizLabel((HorizPosition) v))
                .setSaveConsumer(v -> buyHorizPosition = v).build());
        buySellCat.addEntry(new ColorSwatchEntry(Text.literal("Buy Color"), buyColor, 0x55FF55,
                buyBold, false, v -> buyColor = v, v -> buyBold = v));
        buySellCat.addEntry(eb.startBooleanToggle(Text.literal("Buy Background"), showBuyBackground)
                .setDefaultValue(true).setSaveConsumer(v -> showBuyBackground = v).build());
        buySellCat.addEntry(new ColorSwatchEntry(Text.literal("Buy Background Color"),
                buyBgColor, 0x000000, v -> buyBgColor = v));
        buySellCat.addEntry(eb.startEnumSelector(Text.literal("Buy Size"), PriceSize.class, buySize)
                .setDefaultValue(PriceSize.SMALL).setEnumNameProvider(v -> sizeLabel((PriceSize) v))
                .setSaveConsumer(v -> buySize = v).build());
        buySellCat.addEntry(eb.startBooleanToggle(Text.literal("Show Sell Price"), showSellPrice)
                .setDefaultValue(true).setSaveConsumer(v -> showSellPrice = v).build());
        buySellCat.addEntry(eb.startEnumSelector(Text.literal("Sell Position Height"), TimerPosition.class, sellPosition)
                .setDefaultValue(TimerPosition.BOTTOM).setEnumNameProvider(v -> posLabel((TimerPosition) v))
                .setSaveConsumer(v -> sellPosition = v).build());
        buySellCat.addEntry(eb.startEnumSelector(Text.literal("Sell Position Width"), HorizPosition.class, sellHorizPosition)
                .setDefaultValue(HorizPosition.LEFT).setEnumNameProvider(v -> horizLabel((HorizPosition) v))
                .setSaveConsumer(v -> sellHorizPosition = v).build());
        buySellCat.addEntry(new ColorSwatchEntry(Text.literal("Sell Color"), sellColor, 0xFF5555,
                sellBold, false, v -> sellColor = v, v -> sellBold = v));
        buySellCat.addEntry(eb.startBooleanToggle(Text.literal("Sell Background"), showSellBackground)
                .setDefaultValue(true).setSaveConsumer(v -> showSellBackground = v).build());
        buySellCat.addEntry(new ColorSwatchEntry(Text.literal("Sell Background Color"),
                sellBgColor, 0x000000, v -> sellBgColor = v));
        buySellCat.addEntry(eb.startEnumSelector(Text.literal("Sell Size"), PriceSize.class, sellSize)
                .setDefaultValue(PriceSize.SMALL).setEnumNameProvider(v -> sizeLabel((PriceSize) v))
                .setSaveConsumer(v -> sellSize = v).build());

        // ── Shards ────────────────────────────────────────────────────────────
        ConfigCategory shardsCat = builder.getOrCreateCategory(Text.literal("Shards"));
        shardsCat.addEntry(eb.startBooleanToggle(Text.literal("Toggle Shards"), showShards)
                .setDefaultValue(true).setSaveConsumer(v -> { if (v != initShowShards) showShards = v; }).build());
        shardsCat.addEntry(eb.startEnumSelector(Text.literal("Position Height"), TimerPosition.class, shardsPosition)
                .setDefaultValue(TimerPosition.TOP).setEnumNameProvider(v -> posLabel((TimerPosition) v))
                .setSaveConsumer(v -> shardsPosition = v).build());
        shardsCat.addEntry(eb.startEnumSelector(Text.literal("Position Width"), HorizPosition.class, shardsHorizPosition)
                .setDefaultValue(HorizPosition.LEFT).setEnumNameProvider(v -> horizLabel((HorizPosition) v))
                .setSaveConsumer(v -> shardsHorizPosition = v).build());
        shardsCat.addEntry(eb.startEnumSelector(Text.literal("Display Symbol (S)"), SymbolSide.class, shardsSymbolSide)
                .setDefaultValue(SymbolSide.OFF).setEnumNameProvider(v -> symLabel((SymbolSide) v))
                .setTooltip(Text.literal("Left: S109k | Right: 109kS | Off: 109k"))
                .setSaveConsumer(v -> shardsSymbolSide = v).build());
        shardsCat.addEntry(eb.startEnumSelector(Text.literal("Size"), PriceSize.class, shardsSize)
                .setDefaultValue(PriceSize.SMALL).setEnumNameProvider(v -> sizeLabel((PriceSize) v))
                .setSaveConsumer(v -> shardsSize = v).build());
        shardsCat.addEntry(new ColorSwatchEntry(Text.literal("Digit Color (0=auto)"),
                shardsDigitColor, 0, shardsBold, false, v -> shardsDigitColor = v, v -> shardsBold = v));
        shardsCat.addEntry(new ColorSwatchEntry(Text.literal("Symbol Color"),
                shardsSymbolColor, 0xFF55FF, v -> shardsSymbolColor = v));
        shardsCat.addEntry(eb.startBooleanToggle(Text.literal("Background"), showShardsBackground)
                .setDefaultValue(true).setSaveConsumer(v -> showShardsBackground = v).build());
        shardsCat.addEntry(new ColorSwatchEntry(Text.literal("Background Color"),
                shardsBgColor, 0x000000, v -> shardsBgColor = v));

        // ── Coins ─────────────────────────────────────────────────────────────
        ConfigCategory coinsCat = builder.getOrCreateCategory(Text.literal("Coins"));
        coinsCat.addEntry(eb.startBooleanToggle(Text.literal("Toggle Coins"), showCoins)
                .setDefaultValue(true).setSaveConsumer(v -> { if (v != initShowCoins) showCoins = v; }).build());
        coinsCat.addEntry(eb.startEnumSelector(Text.literal("Position Height"), TimerPosition.class, coinsPosition)
                .setDefaultValue(TimerPosition.TOP).setEnumNameProvider(v -> posLabel((TimerPosition) v))
                .setSaveConsumer(v -> coinsPosition = v).build());
        coinsCat.addEntry(eb.startEnumSelector(Text.literal("Position Width"), HorizPosition.class, coinsHorizPosition)
                .setDefaultValue(HorizPosition.LEFT).setEnumNameProvider(v -> horizLabel((HorizPosition) v))
                .setSaveConsumer(v -> coinsHorizPosition = v).build());
        coinsCat.addEntry(eb.startEnumSelector(Text.literal("Display Symbol (C)"), SymbolSide.class, coinsSymbolSide)
                .setDefaultValue(SymbolSide.OFF).setEnumNameProvider(v -> symLabel((SymbolSide) v))
                .setTooltip(Text.literal("Left: C109k | Right: 109kC | Off: 109k"))
                .setSaveConsumer(v -> coinsSymbolSide = v).build());
        coinsCat.addEntry(eb.startEnumSelector(Text.literal("Size"), PriceSize.class, coinsSize)
                .setDefaultValue(PriceSize.SMALL).setEnumNameProvider(v -> sizeLabel((PriceSize) v))
                .setSaveConsumer(v -> coinsSize = v).build());
        coinsCat.addEntry(new ColorSwatchEntry(Text.literal("Digit Color (0=auto)"),
                coinsDigitColor, 0, coinsBold, false, v -> coinsDigitColor = v, v -> coinsBold = v));
        coinsCat.addEntry(new ColorSwatchEntry(Text.literal("Symbol Color"),
                coinsSymbolColor, 0xFFAA00, v -> coinsSymbolColor = v));
        coinsCat.addEntry(eb.startBooleanToggle(Text.literal("Background"), showCoinsBackground)
                .setDefaultValue(true).setSaveConsumer(v -> showCoinsBackground = v).build());
        coinsCat.addEntry(new ColorSwatchEntry(Text.literal("Background Color"),
                coinsBgColor, 0x000000, v -> coinsBgColor = v));

        // ── Rubies ────────────────────────────────────────────────────────────
        ConfigCategory rubiesCat = builder.getOrCreateCategory(Text.literal("Rubies"));
        rubiesCat.addEntry(eb.startBooleanToggle(Text.literal("Toggle Rubies"), showRubies)
                .setDefaultValue(false).setSaveConsumer(v -> showRubies = v).build());
        rubiesCat.addEntry(eb.startEnumSelector(Text.literal("Position Height"), TimerPosition.class, rubiesPosition)
                .setDefaultValue(TimerPosition.TOP).setEnumNameProvider(v -> posLabel((TimerPosition) v))
                .setSaveConsumer(v -> rubiesPosition = v).build());
        rubiesCat.addEntry(eb.startEnumSelector(Text.literal("Position Width"), HorizPosition.class, rubiesHorizPosition)
                .setDefaultValue(HorizPosition.LEFT).setEnumNameProvider(v -> horizLabel((HorizPosition) v))
                .setSaveConsumer(v -> rubiesHorizPosition = v).build());
        rubiesCat.addEntry(eb.startEnumSelector(Text.literal("Display Symbol (R)"), SymbolSide.class, rubiesSymbolSide)
                .setDefaultValue(SymbolSide.RIGHT).setEnumNameProvider(v -> symLabel((SymbolSide) v))
                .setTooltip(Text.literal("Left: R109k | Right: 109kR | Off: 109k"))
                .setSaveConsumer(v -> rubiesSymbolSide = v).build());
        rubiesCat.addEntry(eb.startEnumSelector(Text.literal("Size"), PriceSize.class, rubiesSize)
                .setDefaultValue(PriceSize.SMALL).setEnumNameProvider(v -> sizeLabel((PriceSize) v))
                .setSaveConsumer(v -> rubiesSize = v).build());
        rubiesCat.addEntry(new ColorSwatchEntry(Text.literal("Digit Color (0=auto)"),
                rubiesDigitColor, 0, rubiesBold, false, v -> rubiesDigitColor = v, v -> rubiesBold = v));
        rubiesCat.addEntry(new ColorSwatchEntry(Text.literal("Symbol Color"),
                rubiesSymbolColor, 0xFF55FF, v -> rubiesSymbolColor = v));
        rubiesCat.addEntry(eb.startBooleanToggle(Text.literal("Background"), showRubiesBackground)
                .setDefaultValue(true).setSaveConsumer(v -> showRubiesBackground = v).build());
        rubiesCat.addEntry(new ColorSwatchEntry(Text.literal("Background Color"),
                rubiesBgColor, 0x000000, v -> rubiesBgColor = v));

        // ── Gems ──────────────────────────────────────────────────────────────
        ConfigCategory gemsCat = builder.getOrCreateCategory(Text.literal("Gems"));
        gemsCat.addEntry(eb.startBooleanToggle(Text.literal("Toggle Gems"), showGems)
                .setDefaultValue(true).setSaveConsumer(v -> showGems = v).build());
        gemsCat.addEntry(eb.startEnumSelector(Text.literal("Position Height"), TimerPosition.class, gemsPosition)
                .setDefaultValue(TimerPosition.TOP).setEnumNameProvider(v -> posLabel((TimerPosition) v))
                .setSaveConsumer(v -> gemsPosition = v).build());
        gemsCat.addEntry(eb.startEnumSelector(Text.literal("Position Width"), HorizPosition.class, gemsHorizPosition)
                .setDefaultValue(HorizPosition.LEFT).setEnumNameProvider(v -> horizLabel((HorizPosition) v))
                .setSaveConsumer(v -> gemsHorizPosition = v).build());
        gemsCat.addEntry(eb.startEnumSelector(Text.literal("Display Symbol (G)"), SymbolSide.class, gemsSymbolSide)
                .setDefaultValue(SymbolSide.OFF).setEnumNameProvider(v -> symLabel((SymbolSide) v))
                .setTooltip(Text.literal("Left: G109k | Right: 109kG | Off: 109k"))
                .setSaveConsumer(v -> gemsSymbolSide = v).build());
        gemsCat.addEntry(eb.startEnumSelector(Text.literal("Size"), PriceSize.class, gemsSize)
                .setDefaultValue(PriceSize.SMALL).setEnumNameProvider(v -> sizeLabel((PriceSize) v))
                .setSaveConsumer(v -> gemsSize = v).build());
        gemsCat.addEntry(new ColorSwatchEntry(Text.literal("Digit Color (0=auto)"),
                gemsDigitColor, 0, gemsBold, false, v -> gemsDigitColor = v, v -> gemsBold = v));
        gemsCat.addEntry(new ColorSwatchEntry(Text.literal("Symbol Color"),
                gemsSymbolColor, 0x00AA00, v -> gemsSymbolColor = v));
        gemsCat.addEntry(eb.startBooleanToggle(Text.literal("Background"), showGemsBackground)
                .setDefaultValue(true).setSaveConsumer(v -> showGemsBackground = v).build());
        gemsCat.addEntry(new ColorSwatchEntry(Text.literal("Background Color"),
                gemsBgColor, 0x000000, v -> gemsBgColor = v));

        // ── Keys ──────────────────────────────────────────────────────────────
        ConfigCategory keysCat = builder.getOrCreateCategory(Text.literal("Keys"));
        keysCat.addEntry(eb.startBooleanToggle(Text.literal("Show Key Count"), showKeyCount)
                .setDefaultValue(true)
                .setTooltip(Text.literal("Display key/balance count from tooltips (e.g. '5 Keys', 'You have 0 Keys')"))
                .setSaveConsumer(v -> { if (v != initShowKeyCount) showKeyCount = v; }).build());
        keysCat.addEntry(eb.startBooleanToggle(Text.literal("Show Key Label"), showKeyLabel)
                .setDefaultValue(false)
                .setTooltip(Text.literal("Off: shows digit only. On: shows '5 Keys'"))
                .setSaveConsumer(v -> showKeyLabel = v).build());
        keysCat.addEntry(eb.startEnumSelector(Text.literal("Position Height"), TimerPosition.class, keyPosition)
                .setDefaultValue(TimerPosition.BOTTOM).setEnumNameProvider(v -> posLabel((TimerPosition) v))
                .setSaveConsumer(v -> keyPosition = v).build());
        keysCat.addEntry(eb.startEnumSelector(Text.literal("Position Width"), HorizPosition.class, keyHorizPosition)
                .setDefaultValue(HorizPosition.LEFT).setEnumNameProvider(v -> horizLabel((HorizPosition) v))
                .setSaveConsumer(v -> keyHorizPosition = v).build());
        keysCat.addEntry(eb.startEnumSelector(Text.literal("Size"), PriceSize.class, keySize)
                .setDefaultValue(PriceSize.SMALL).setEnumNameProvider(v -> sizeLabel((PriceSize) v))
                .setSaveConsumer(v -> keySize = v).build());
        keysCat.addEntry(new ColorSwatchEntry(Text.literal("Key Count Color (0=auto)"),
                keyCountColor, 0x55FFFF, keyCountBold, false, v -> keyCountColor = v, v -> keyCountBold = v));
        keysCat.addEntry(eb.startBooleanToggle(Text.literal("Background"), showKeyBackground)
                .setDefaultValue(true).setSaveConsumer(v -> showKeyBackground = v).build());
        keysCat.addEntry(new ColorSwatchEntry(Text.literal("Background Color"),
                keyBgColor, 0x000000, v -> keyBgColor = v));
        // Key Cost
        keysCat.addEntry(eb.startBooleanToggle(Text.literal("Show Key Cost"), showKeyCost)
                .setDefaultValue(true)
                .setTooltip(Text.literal("Display key cost from 'Cost: X keys' in tooltips"))
                .setSaveConsumer(v -> showKeyCost = v).build());
        keysCat.addEntry(eb.startEnumSelector(Text.literal("Key Cost Position Height"), TimerPosition.class, keyCostVertPosition)
                .setDefaultValue(TimerPosition.TOP).setEnumNameProvider(v -> posLabel((TimerPosition) v))
                .setSaveConsumer(v -> keyCostVertPosition = v).build());
        keysCat.addEntry(eb.startEnumSelector(Text.literal("Key Cost Position Width"), HorizPosition.class, keyCostHorizPosition)
                .setDefaultValue(HorizPosition.RIGHT).setEnumNameProvider(v -> horizLabel((HorizPosition) v))
                .setSaveConsumer(v -> keyCostHorizPosition = v).build());
        keysCat.addEntry(eb.startEnumSelector(Text.literal("Key Cost Size"), PriceSize.class, keyCostSize)
                .setDefaultValue(PriceSize.SMALL).setEnumNameProvider(v -> sizeLabel((PriceSize) v))
                .setSaveConsumer(v -> keyCostSize = v).build());
        keysCat.addEntry(new ColorSwatchEntry(Text.literal("Key Cost Color"), keyCostColor, 0xFF5555,
                keyCostBold, false, v -> keyCostColor = v, v -> keyCostBold = v));
        keysCat.addEntry(eb.startBooleanToggle(Text.literal("Key Cost Background"), showKeyCostBackground)
                .setDefaultValue(true).setSaveConsumer(v -> showKeyCostBackground = v).build());
        keysCat.addEntry(new ColorSwatchEntry(Text.literal("Key Cost BG Color"),
                keyCostBgColor, 0x000000, v -> keyCostBgColor = v));

        // ── Chance ────────────────────────────────────────────────────────────
        ConfigCategory chanceCat = builder.getOrCreateCategory(Text.literal("Chance"));
        chanceCat.addEntry(eb.startBooleanToggle(Text.literal("Show Chance"), showChance)
                .setDefaultValue(true)
                .setTooltip(Text.literal("Detect and display 'Chance: X%' or any X% in tooltips"))
                .setSaveConsumer(v -> showChance = v).build());
        chanceCat.addEntry(eb.startBooleanToggle(Text.literal("Show Symbol (%)"), showChanceSymbol)
                .setDefaultValue(true)
                .setTooltip(Text.literal("Show or hide the % symbol after the number"))
                .setSaveConsumer(v -> showChanceSymbol = v).build());
        chanceCat.addEntry(eb.startEnumSelector(Text.literal("Position Height"), TimerPosition.class, chanceVertPosition)
                .setDefaultValue(TimerPosition.BOTTOM).setEnumNameProvider(v -> posLabel((TimerPosition) v))
                .setSaveConsumer(v -> chanceVertPosition = v).build());
        chanceCat.addEntry(eb.startEnumSelector(Text.literal("Position Width"), HorizPosition.class, chanceHorizPosition)
                .setDefaultValue(HorizPosition.RIGHT).setEnumNameProvider(v -> horizLabel((HorizPosition) v))
                .setSaveConsumer(v -> chanceHorizPosition = v).build());
        chanceCat.addEntry(eb.startEnumSelector(Text.literal("Size"), PriceSize.class, chanceSize)
                .setDefaultValue(PriceSize.MEDIUM).setEnumNameProvider(v -> sizeLabel((PriceSize) v))
                .setSaveConsumer(v -> chanceSize = v).build());
        chanceCat.addEntry(new ColorSwatchEntry(Text.literal("Color (0=auto from tooltip)"),
                chanceColor, 0xFFFF55, chanceBold, false, v -> chanceColor = v, v -> chanceBold = v));
        chanceCat.addEntry(new ColorSwatchEntry(Text.literal("Symbol Color (0=auto)"),
                chanceSymbolColor, 0, v -> chanceSymbolColor = v));
        chanceCat.addEntry(eb.startBooleanToggle(Text.literal("Background"), showChanceBackground)
                .setDefaultValue(false).setSaveConsumer(v -> showChanceBackground = v).build());
        chanceCat.addEntry(new ColorSwatchEntry(Text.literal("Background Color"),
                chanceBgColor, 0x000000, v -> chanceBgColor = v));
        chanceCat.addEntry(eb.startBooleanToggle(Text.literal("Fraction Mode"), chanceFractionMode)
                .setDefaultValue(false)
                .setTooltip(Text.literal("Convert percentage to fraction: 0.01% → 1/10k"))
                .setSaveConsumer(v -> chanceFractionMode = v).build());
        chanceCat.addEntry(new ColorSwatchEntry(Text.literal("Numerator Color (0=auto)"),
                chanceNumeratorColor, 0, v -> chanceNumeratorColor = v));
        chanceCat.addEntry(new ColorSwatchEntry(Text.literal("Denominator Color (0=auto)"),
                chanceDenominatorColor, 0, v -> chanceDenominatorColor = v));
        chanceCat.addEntry(new ColorSwatchEntry(Text.literal("Slash Color (0=auto)"),
    ...
    public static Text posLabel(TimerPosition p) {
        return switch (p) {
    ...
    public static Text sizeLabel(PriceSize s) {
        return switch (s) {
            case XSMALL -> Text.literal("XSmall");
            case SMALL  -> Text.literal("Small");
            case MEDIUM -> Text.literal("Medium");
    ...
    private static Text symLabel(SymbolSide s) {
        return switch (s) {
    ...
    public static Text horizLabel(HorizPosition h) {
        return switch (h) {
            case LEFT   -> Text.literal("Left");
            case MIDDLE -> Text.literal("Middle");
            case RIGHT  -> Text.literal("Right");
