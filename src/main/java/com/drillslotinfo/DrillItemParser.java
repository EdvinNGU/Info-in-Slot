...
public class DrillItemParser {
    ...
    public static ParsedDrillData getData(ItemStack stack) {
        ...
        for (int i = 1; i < tooltip.size(); i++) {
            Component lineText = tooltip.get(i);
    ...
    static int extractBestColor(Component lineText, String rawWithCodes, String strippedTrimmed, int strippedPos) {
    ...
    private static int extractColorFromTextComponent(Component text, int targetStrippedPos) {
        int[] pos   = {0};
        int[] found = {0xFFFFFF};
        text.visit((Style style, String s) -> {
            ...
            if (targetStrippedPos >= pos[0] && targetStrippedPos < end) {
                if (style.getColor() != null) found[0] = style.getColor().getValue();
